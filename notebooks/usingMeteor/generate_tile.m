...
% for i = 1:numel(custom_list) %numel(country_path)
    ...
    [~,~] = mkdir(path_i+"tiles/extents");  % global
    [~,~] = mkdir(path_i+"tiles/images");   % global
    [~,~] = mkdir(path_i+"tiles/natl/extents");  % national
    [~,~] = mkdir(path_i+"tiles/natl/images");   % national
    


    % initialize map_nlabel - natl and glbl
    map_nlabel = zeros(size(loc,1),size(loc,2),numel(bldgtype));

    % select building type 'j' - natl and glbl
    ...
    % load country mask - natl and glbl
    ...
    % representation of all labels) - greedy dynamic search - natl and glbl
    ind = [];
    attr = [];
    nzeros_ratio = [];
    ...
    % second fix - number of extracted tiles is enough but selected rows - glbl only
    % are just limiting
    ...
    for k = 1:size(ind,1)
        ...
        %                  path_i+loc_raster_file(1:3)+"_tile_geoextents.csv",'Delimiter',',') % glbl
        writetable(table(ll_lot_list, ll_lat_list, lr_lot_list, lr_lat_list, ...
                         ur_lot_list, ur_lat_list, ul_lot_list, ul_lat_list), ...
                         path_i+loc_raster_file(1:3)+"_tile_geoextents_natl.csv",'Delimiter',',') % natl
        ...
        %     num2str(max(col))+...
        %     '.geojson','w'); % glbl
        fileID = fopen(path_i+"tiles/natl/extents/"+ ...
            loc_raster_file(1:3)+"_"+num2str(k)+"_of_"+num2str(size(ind,1))+"_"+...
            num2str(min(row))+"_"+...
            num2str(max(row))+"_"+...
            num2str(min(col))+"_"+...
        ...
        for m = 1:numel(bldgtype)
            ...
            [map_nbldg_crop,~] = geocrop(...
                ...
                [ceil(ll_lat*1000)/1000 floor(ul_lat*1000)/1000], ...
                [ceil(ll_lot*1000)/1000 floor(lr_lot*1000)/1000]);
            geotiffwrite(path_i+"tiles/natl/images/"+ ...
                loc_raster_file(1:3)+"_nbldg_"+string(bldgtype{m,1})+"_"+ ...
                num2str(k)+"_of_"+num2str(size(ind,1))+"_"+...
                num2str(min(row))+"_"+...
                num2str(max(row))+"_"+...
