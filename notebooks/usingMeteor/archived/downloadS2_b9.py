...
for ic in range(len(country_list)): 
    ...
    for ifilename in range(len(filenamelist)): # range(custom start index,len(filenamelist)):
        ...
        for i in range(len(uniq_year)):
            ...
            if not os.path.isfile(str(result_path+'/'+str(uniq_year[i])+"_B9_vapor.tif")) or (os.path.getsize(str(result_path+'/'+str(uniq_year[i])+"_B9_vapor.tif"))/(1<<10)) < 1:
                ims1.append(im_coll.filterDate(startDATE,endDATE).select('B9').mean().clip(aoi))
                fns1.append(str(result_path+'/'+str(uniq_year[i])+"_B9_vapor.tif"))
                rgns1.append(region)
