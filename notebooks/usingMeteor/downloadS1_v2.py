...
for ic in range(len(country_list)): #range(2, 41): # len(country_list)):
    ...
    for ifilename in range(len(filenamelist)): # range(custom start index,len(filenamelist)):
        ...
        for i in range(len(uniq_year)):
            ...
            if not os.path.isfile(str(result_path+'/'+str(uniq_year[i])+"_VH.tif")) or (os.path.getsize(str(result_path+'/'+str(uniq_year[i])+"_VH.tif"))/(1<<10)) < 1 or not os.path.isfile(str(result_path+'/'+str(uniq_year[i])+"_VV.tif")) or (os.path.getsize(str(result_path+'/'+str(uniq_year[i])+"_VV.tif"))/(1<<10)) < 1:
